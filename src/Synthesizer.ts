...
export class YandexTextToSpeech implements Synthesizer {
  constructor(private accessKey: string, private folderId: string) {
    this.setToken();
  }
  ...
  async synthesize(
    ...
    return axios
      .post(url, data, { headers: headers, responseType: "arraybuffer" })
      .then(response => {
